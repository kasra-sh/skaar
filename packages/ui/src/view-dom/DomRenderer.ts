...
import { IComponentUpdater } from '../component/IComponentUpdater';
import { FnComponent } from '../component/FnComponent';
import { IViewRenderer } from '../view/IViewRenderer';
...
export class DomRenderer implements IViewRenderer, IComponentUpdater {
   ...
   updateVElementEvents(newEl: VElement, oldEl: VElement, parentComponent: Component | FnComponent) {
      if (JSON.stringify(newEl.props.events) === JSON.stringify(oldEl.props.events)) return;
      oldEl._cleanup();
    ...
    */
   updateQueue = Array<{
      partialState: JsObject;
      component: Component | FnComponent;
      mergeCount: number;
   }>();
   isUpdating = false;
   isDisposed = false;
   ...
   startUpdating() {
      if (!this.isUpdating) {
         ...
         if (requestAnimationFrame) requestAnimationFrame(() => this.applyUpdates(callback));
   ...
   render(vnode: VNode | VNodeContainer | Function, viewNode: HTMLElement) {
      if ((vnode as any)._type === undefined) {
         vnode = createNode(vnode as any);
      }
