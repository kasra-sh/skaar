...
export function useEffect(effect: EffectFn, deps?: Array<any>) {
   const currentComponent = FncContext.current.component;
   const [state, setState] = useState<UseEffectState>({
      cleanup: undefined,
      deps: undefined,
   });
   if (state.cleanup) {
   ...
   if (shouldRun) {
      ...
      if (typeof cleanup === 'function') currentComponent._cleaners.push(cleanup);
   }
...
export function useMemo<T>(factory: () => T, deps?: Array<any>): T {
   const [state, setState] = useState<UseMemoState<T>>({
