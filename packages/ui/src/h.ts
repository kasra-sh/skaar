...
import { transformJsxProperties } from './transformJsxProperties';
...
export function h(type: null | string | ComponentClass | Component, props?: JsObject, children?: (VNode | string | unknown)[]): Fragment | VElement | Component {
   if (type === null) {
      return new Fragment(children);
   }
   if (typeof type === 'string') {
      return createVElement(type, transformJsxProperties(props as VElementProps), children);
   }
   if (typeof type === 'function') return createComponent(type, props, children);
   else {
